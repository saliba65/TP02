...
int compareTo(char *avatar_1, char *avatar_2, char *avatar_3, char *avatar_4)
{
    int resposta = 0;
    int tamanho = strlen(avatar_1);

    if (strlen(avatar_2) < tamanho)
    {
        tamanho = strlen(avatar_2);
    }
    for (int i = 0; i < tamanho; i++)
    {
        if (avatar_1[i] > avatar_2[i])
        {
            resposta = 1;
            i = tamanho;
        }

        else if (avatar_1[i] < avatar_2[i])
        {
            resposta = -1;
            i = tamanho;
        }
    ...
    if (resposta == 0 && strlen(avatar_1) != strlen(avatar_2))
    {
        if (strlen(avatar_2) > strlen(avatar_1))
        {
            resposta = -1;
        }
        else
