...
class ListaSequencial {
    ...
    public boolean pesquisaSequencial(String nome) throws Exception {
        boolean resposta = false;
        String nomePersonagem = "";
        for (int i = 0; i < contador; i++) {
            // personagem[i].readCharacter();
            nomePersonagem = personagem[i].getName();
            if (nome.compareTo(nomePersonagem) == 0) {
                i = contador;
                resposta = true;
            }
        }
...
class PesquisaSequencial {
    ...
    public static void main(String[] args) throws Exception {
        ...
        do {
            if (list.pesquisaSequencial(nome)) {
