...
int compareTo(int a, int b, char* c, char* d);
...
{
    for(int i = tamHeap; i > 1 && compareTo(array[i].altura, array[i/2].altura, array[i].nome, array[i/2].nome) > 0; i /= 2)
    {
        swap(i, i/2);
    }
}
...
void reconstroi(int tamHeap)
{
    ...
    while(i <= (tamHeap/2))
    {
        ...
        if (compareTo(array[2*i].altura, array[2*i+1].altura, array[2*i].nome, array[2*i+1].nome) > 0 || 2*i == tamHeap)
        {
            filho = 2*i;
        } 
        else 
