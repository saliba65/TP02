...
void quicksortRec(int esq, int dir)
{
    ...
    while (i <= j)
    {
        while (compareTo(array[i].corDoCabelo, pivo.corDoCabelo, array[i].nome, pivo.nome) < 0)
            i++;
        while (compareTo(array[j].corDoCabelo, pivo.corDoCabelo, array[j].nome, pivo.nome) > 0)
            j--;
