...
class ListaSequencial {
    ...
    public int compare(Characters jedi, Characters sith) {

        int resp = 0;

        String hair_1 = jedi.getHairColor();
        String hair_2 = sith.getHairColor();

        int tam = hair_1.length();
        if (hair_2.length() < tam) {
            tam = hair_2.length();
        }

        for (int i = 0; i < tam; i++) {

            if (hair_1.charAt(i) > hair_2.charAt(i)) {
                resp = 1;
                i = tam;
            }

            else if (hair_1.charAt(i) < hair_2.charAt(i)) {
                resp = -1;
                i = tam;
            }
        }

        if (resp == 0 && hair_1.length() != hair_2.length()) {
            if (hair_1.length() > hair_2.length()) {
                resp = -1;
            } else {
                resp = 1;
            }
        ...
        if (resp == 0) {
            ...
            String name_1 = jedi.getName();
            String name_2 = sith.getName();

            tam = name_1.length();
            if (name_2.length() < tam)
                tam = name_2.length();
            ...
            for (int i = 0; i < tam; i++) {
                ...
                else if (name_1.charAt(i) < name_2.charAt(i)) {
                    resp = -1;
                    i = tam;
                }
            }

            if (resp == 0 && name_1.length() != name_2.length()) {
    ...
    public void quicksort(int esq, int dir) {
        int i = esq, j = dir;
        Characters pivo = personagem[(dir + esq) / 2];

        while (i <= j) {
            while (compare(pivo, personagem[i]) > 0)
                i++;
            while (compare(personagem[j], pivo) > 0)
                j--;
            if (i <= j) {
                swap(i, j);
                i++;
